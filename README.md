...
В кластере `core`, PostgreSQL и RabbitMQ работают на ноде в РФ, `telegram-bot` и `agent-adapter` — на зарубежной. Зарубежные сервисы не имеют доступа к базе, ничего не сохраняют и не пишут в логи содержимое сообщений и фото. Манифесты и порядок выката — [deploy/k8s/README.md](deploy/k8s/README.md).
