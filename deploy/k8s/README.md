...
(нода РФ, рядом с базой), проверяет дамп через `pg_restore --list`, удаляет копии старше `KEEP_DAYS` (двое суток)
...
`karandash`, `chimney`, `tracker`; хранилище само удаляет копии старше двух суток. У каждого сервиса
...
Локальная копия на томе рядом с базой тоже остаётся (двое суток): с ней восстановление быстрее.
