...
Код зарубежных сервисов не держит данных и не имеет доступа к базе, но сетевая изоляция в кластере пока не включена. До запуска с настоящими пользователями:

- [ ] Трафик между нодами зашифрован: flannel `wireguard-native` или TLS между сервисами. Иначе текст, фото и сервисные токены идут между странами открыто.
...
- [ ] Пользователь RabbitMQ `karandash-bot` создан (см. ниже) и заведён в бэкап или definitions: при пересоздании брокера приёмщик иначе потеряет доступ к очереди.
