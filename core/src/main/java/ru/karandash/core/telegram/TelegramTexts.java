...
final class TelegramTexts {
    ...
    static final String HOWTO = """
            ...
            Фото еды. Сфотографируйте тарелку целиком, сверху или под углом, чтобы было видно всё, \
            что вы едите. Одно фото — один приём пищи. Подпись к фото помогает: «двойная порция», \
            «без масла», «это половина».

            Что я делаю с фото. Распознаю блюда и прикидываю вес порций, считаю калории и БЖУ \
            диапазоном — точного числа у еды на глаз не бывает. По будням сверяю увиденное \
            с сегодняшним меню столовой: если блюдо оттуда, беру его вес и калорийность из меню.

            Словами тоже можно. «Гречка с курицей, обычная тарелка», «два яйца и тост». \
            Чем понятнее порция, тем точнее оценка.

            Оценка не попадает в дневник сама. Под ней две кнопки: «Записать в дневник» \
            и «Не записывать». Если оценка мимо — просто напишите, что не так, и я пересчитаю ту же еду.

            Что взять поесть. Спросите словами: «что мне поесть на 300 калорий», «чем пообедать» — \
            подберу из сегодняшнего меню столовой с фотографиями блюд.

            Напоминания и серия. Напомню записать завтрак, обед и ужин, а вечером пришлю итоги дня. \
            Серия — это дни подряд, когда вы записали хотя бы одну еду; пропустить можно не больше \
            трёх дней подряд.

            Ничего из ваших фотографий и переписки я не храню: в дневник попадают только названия \
            блюд и числа.""";
